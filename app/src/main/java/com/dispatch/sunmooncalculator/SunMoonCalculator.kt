 ...
 * @version November 10, 2020 (better refraction correction, still with Bennet's formula)
...
class SunMoonCalculator(
     ...
     * Note in the [Ephemeris] fields rise/set times
     * are still returned in UT.  */
    enum class TWILIGHT_MODE {
        /** Closest events.  */
        CLOSEST,
        ...
        companion object {
            /** Time zone for option [.TODAY_LT], LT-UT, hours.  */
            var timeZone = 0
        }
    }
    ...
    enum class BODY(
        ...
        MERCURY(0, 2439.7), VENUS(1, 6051.8), MARS(3, 3396.19), JUPITER(4, 71492.0), SATURN(
            5,
            60268.0
        ),
        URANUS(6, 25559.0), NEPTUNE(7, 24764.0), Moon(-2, 1737.4), Sun(-1, 696000.0), EMB(2, 0.0);
    }

    /** Input values and nutation/obliquity parameters only calculated once.  */
    private var obsLon = 0.0
    private var obsLat = 0.0
    private var twilight = TWILIGHT.HORIZON_34arcmin
    private var twilightMode = TWILIGHT_MODE.CLOSEST
    protected var jd_UT = 0.0
    protected var t = 0.0
    protected var nutLon = 0.0
    protected var nutObl = 0.0
    protected var meanObliquity = 0.0
    protected var TTminusUT = 0.0
    protected var lst = 0.0

    /**
     * Class to hold the results of ephemerides.
     * @author T. Alonso Albi - OAN (Spain)
     */
    inner class Ephemeris constructor(
     ...
     * @p
     * aram obsLat Latitude for the observer.
     */
    fun SunMoonCalculator(
        year: Int, month: Int, day: Int, h: Int, m: Int, s: Int,
        obsLon: Double, obsLat: Double
    ) {
        val jd = toJulianDay(year, month, day, h, m, s)
        val ndot = -25.858
        var c0 = 0.91072 * (ndot + 26.0)
        if (year < -500 || year >= 2200) {
            val u = (jd - 2385800.5) / 36525.0 // centuries since J1820
            TTminusUT = -20 + 32.0 * u * u
        } else {
            val x = year + (month - 1 + (day - 1) / 30.0) / 12.0
            val x2 = x * x
            val x3 = x2 * x
            val x4 = x3 * x
            TTminusUT = if (year < 1600) {
                10535.328003 - 9.9952386275 * x + 0.00306730763 * x2 - 7.7634069836E-6 * x3 + 3.1331045394E-9 * x4 + 8.2255308544E-12 * x2 * x3 - 7.4861647156E-15 * x4 * x2 + 1.936246155E-18 * x4 * x3 - 8.4892249378E-23 * x4 * x4
            } else {
                -1027175.34776 + 2523.2566254 * x - 1.8856868491 * x2 + 5.8692462279E-5 * x3 + 3.3379295816E-7 * x4 + 1.7758961671E-10 * x2 * x3 - 2.7889902806E-13 * x2 * x4 + 1.0224295822E-16 * x3 * x4 - 1.2528102371E-20 * x4 * x4
            }
            c0 = 0.91072 * (ndot + 25.858)
        }
        val c = -c0 * Math.pow((jd - 2435109.0) / 36525.0, 2.0)
     ...
     */
    fun setTwilightMode(t: TWILIGHT_MODE) {
        twilightMode = t
    }

    /**
     * Sets the time zone to use to return rise/set times for today in local time.
     * @param t The time zone (LT-UT in hours) for the twilight mode [TWILIGHT_MODE.TODAY_LT].
     */
    fun setTwilightModeTimeZone(t: Int) {
     ...
     */
    protected fun setUTDate(jd: Double) {
        jd_UT = jd
        ...
        nutLon =
            (-(17.2026 + 0.01737 * t) * Math.sin(M1) + (-1.32012 + 0.00013 * t) * Math.sin(M2) + .2088 * Math.sin(
                2 * M1
            )) * ARCSEC_TO_RAD
        nutObl =
            ((9.2088 + .00091 * t) * Math.cos(M1) + (0.552204 - 0.00029 * t) * Math.cos(M2) - .0904 * Math.cos(
                2 * M1
            )) * ARCSEC_TO_RAD
        ...
        val jd0 = Math.floor(jd_UT - 0.5) + 0.5
        val T0 = (jd0 - J2000) / JULIAN_DAYS_PER_CENTURY
        val secs = (jd_UT - jd0) * SECONDS_PER_DAY
    ...
    fun calcSunAndMoon() {
        ...
        // First the Sun
        sun = doCalc(getSun())
        val niter = 15 // Number of iterations to get accurate rise/set/transit times
        ...
        if (sun!!.transit == -1.0) {
            ...
            setUTDate(sun!!.transit.toDouble())
    ...
    protected fun getSun(): DoubleArray {
        var L = 0.0
        var R = 0.0
        val t2 = t * 0.01
        var Lp = 0.0
        val deltat = 0.5
        val t2p = (t + deltat / JULIAN_DAYS_PER_CENTURY) * 0.01
        for (i in sun_elements.indices) {
            ...
            val vp = sun_elements[i][2] + sun_elements[i][3] * t2p
            val up = normalizeRadians(vp)
            Lp = Lp + sun_elements[i][0] * Math.sin(up)
        }
        var lon =
            normalizeRadians(4.9353929 + normalizeRadians(62833.196168 * t2) + L / 10000000.0) * RAD_TO_DEG
        val sdistance = 1.0001026 + R / 10000000.0

        // Now subtract aberration
        val dlon = ((Lp - L) / 10000000.0 + 62833.196168 * (t2p - t2)) / deltat
        val aberration = dlon * sdistance * LIGHT_TIME_DAYS_PER_AU
        lon -= aberration * RAD_TO_DEG
        val slongitude = lon * DEG_TO_RAD // apparent longitude (error<0.001 deg)
        val slatitude = 0.0 // Sun's ecliptic latitude is always negligible
        return doubleArrayOf(
            slongitude,
            slatitude,
            sdistance,
            Math.atan(BODY.Sun.eqRadius / (AU * sdistance))
        )
    }

    protected fun getMoon(): DoubleArray {
        ...
        val phase = normalizeRadians(ARCSEC_TO_RAD * x)

        /* Mean distance of moon from its ascending node = F */x =
            1.7395272628437717e+09 * t + 3.3577951412884740e+05
        ...
        val node = normalizeRadians(ARCSEC_TO_RAD * x)

        /* Mean anomaly of sun = l' (J. Laskar) */x =
            1.2959658102304320e+08 * t + 1.2871027407441526e+06
        ...
        val sanomaly = normalizeRadians(ARCSEC_TO_RAD * x)

        /* Mean anomaly of moon = l */x = 1.7179159228846793e+09 * t + 4.8586817465825332e+05
        ...
        val anomaly = normalizeRadians(ARCSEC_TO_RAD * x)

        /* Mean longitude of moon, re mean ecliptic and equinox of date = L */x =
            1.7325643720442266e+09 * t + 7.8593980921052420e+05
        ...
        var l = normalizeRadians(ARCSEC_TO_RAD * x) * RAD_TO_DEG
        ...
        l += 6.28875 * Math.sin(anomaly) + 1.274018 * Math.sin(2 * phase - anomaly) + .658309 * Math.sin(
            2 * phase
        )
        l += 0.213616 * Math.sin(2 * anomaly) - E * .185596 * Math.sin(sanomaly) - 0.114336 * Math.sin(
            2 * node
        )
        l += .058793 * Math.sin(2 * phase - 2 * anomaly) + .057212 * E * Math.sin(
            2 * phase - anomaly - sanomaly
        ) + .05332 * Math.sin(2 * phase + anomaly)
        l += .045874 * E * Math.sin(2 * phase - sanomaly) + .041024 * E * Math.sin(anomaly - sanomaly) - .034718 * Math.sin(
            phase
        ) - E * .030465 * Math.sin(sanomaly + anomaly)
        l += .015326 * Math.sin(2 * (phase - node)) - .012528 * Math.sin(2 * node + anomaly) - .01098 * Math.sin(
            2 * node - anomaly
        ) + .010674 * Math.sin(4 * phase - anomaly)
        l += .010034 * Math.sin(3 * anomaly) + .008548 * Math.sin(4 * phase - 2 * anomaly)
        l += -E * .00791 * Math.sin(sanomaly - anomaly + 2 * phase) - E * .006783 * Math.sin(2 * phase + sanomaly) + .005162 * Math.sin(
            anomaly - phase
        ) + E * .005 * Math.sin(sanomaly + phase)
        l += .003862 * Math.sin(4 * phase) + E * .004049 * Math.sin(anomaly - sanomaly + 2 * phase) + .003996 * Math.sin(
            2 * (anomaly + phase)
        ...
        // Now Moon parallax
        var parallax =
            .950724 + .051818 * Math.cos(anomaly) + .009531 * Math.cos(2 * phase - anomaly)
        ...
        parallax += E * .000401 * Math.cos(2 * phase - anomaly - sanomaly) + E * .00032 * Math.cos(
            anomaly - sanomaly
        ) - .000271 * Math.cos(phase)
        ...
        val distance = 1.0 / Math.sin(parallax * DEG_TO_RAD)

        // Ecliptic latitude with nodal phase (error<0.01 deg)
        l = 5.128189 * Math.sin(node) + 0.280606 * Math.sin(node + anomaly) + 0.277693 * Math.sin(
            anomaly - node
        )
        ...
        l += .008823 * Math.sin(2 * anomaly - node) + E * .008247 * Math.sin(2 * phase - sanomaly - node) + .004323 * Math.sin(
        ...
        val latitude = l * DEG_TO_RAD
        return doubleArrayOf(
            longitude,
            latitude,
            distance * EARTH_RADIUS / AU,
            Math.atan(BODY.Moon.eqRadius / (distance * EARTH_RADIUS))
        )
    }
    /**
     ...
     * @return The ephemeris object with the output position
     */
    protected fun doCalc(
        pos: DoubleArray,
        geocentric: Boolean = false
    ): Ephemeris {
        // Correct for nutation in longitude
        pos[0] = pos[0] + nutLon

        // Ecliptic to equatorial coordinates using true obliquity
        val cl = Math.cos(pos[1])
        val x = pos[2] * Math.cos(pos[0]) * cl
        var y = pos[2] * Math.sin(pos[0]) * cl
        var z = pos[2] * Math.sin(pos[1])
        val sinEcl = Math.sin(meanObliquity + nutObl)
        val cosEcl = Math.cos(meanObliquity + nutObl)
        var tmp = y * cosEcl - z * sinEcl
        z = y * sinEcl + z * cosEcl
        y = tmp

        // Obtain topocentric rectangular coordinates
        val sinLat = Math.sin(obsLat)
        val cosLat = Math.cos(obsLat)
        val radiusAU: Double =
            if (geocentric) 0.0 else EARTH_RADIUS / AU
        ...
        val dist =
            Math.sqrt(xtopo * xtopo + ytopo * ytopo + ztopo * ztopo)

        // Hour angle
        val angh = lst - ra

        // Obtain azimuth and geometric alt
        val sinDec = Math.sin(dec)
        val cosDec = Math.cos(dec)
        val h = sinLat * sinDec + cosLat * cosDec * Math.cos(angh)
        var alt = Math.asin(h)
        val azy = Math.sin(angh)
        val azx = Math.cos(angh) * sinLat - sinDec * cosLat / cosDec
        val azi = Math.PI + Math.atan2(azy, azx) // 0 = north
        if (geocentric) return Ephemeris(
            azi,
            alt,
            -1.0,
            -1.0,
        ...
        tmp = when (twilight) {
            ...
            TWILIGHT.CIVIL -> -6 * DEG_TO_RAD
            TWILIGHT.NAUTICAL -> -12 * DEG_TO_RAD
            TWILIGHT.ASTRONOMICAL -> -18 * DEG_TO_RAD
        }
        ...
        val celestialHoursToEarthTime =
            1.0 / (siderealDayLength * TWO_PI)

        // Make calculations for the meridian
        var transit_alt = Math.asin(sinDec * sinLat + cosDec * cosLat)
        transit_alt = refraction(transit_alt)

        // Obtain the current transit event in time
        val transit = getTwilightEvent(celestialHoursToEarthTime, ra, 0.0)

        // Make calculations for rise and set
        var rise = -1.0
        var set = -1.0
        if (Math.abs(tmp) <= 1.0) {
            val ang_hor = Math.abs(Math.acos(tmp))
            rise = getTwilightEvent(celestialHoursToEarthTime, ra, -ang_hor)
            set = getTwilightEvent(celestialHoursToEarthTime, ra, ang_hor)
        }
        return Ephemeris(
            azi, alt, rise, set, transit, transit_alt,
            normalizeRadians(ra), dec, dist, pos[0], pos[1], pos[3]
        )
    }

    private fun getTwilightEvent(
        celestialHoursToEarthTime: Double,
        ra: Double,
        angh: Double
    ): Double {
        val jdToday_UT = Math.floor(jd_UT - 0.5) + 0.5
        var eventTime = celestialHoursToEarthTime * normalizeRadians(ra + angh - lst)
        val eventTimePrev = eventTime - celestialHoursToEarthTime * TWO_PI
        val eventDatePrev_UT = Math.floor(jd_UT + eventTimePrev - 0.5) + 0.5
        if (twilightMode == TWILIGHT_MODE.CLOSEST &&
            Math.abs(eventTimePrev) < Math.abs(eventTime)
        ) eventTime = eventTimePrev
        if (twilightMode == TWILIGHT_MODE.TODAY_UT) {
            val eventDate_UT = Math.floor(jd_UT + eventTime - 0.5) + 0.5
        ...
        if (twilightMode == TWILIGHT_MODE.TODAY_LT) {
            val tz = TWILIGHT_MODE.timeZone / 24.0
            val jdToday_LT = Math.floor(jd_UT + tz - 0.5) + 0.5
            val eventDate_LT = Math.floor(jd_UT + tz + eventTime - 0.5) + 0.5
            if (jdToday_LT != eventDate_LT) eventTime = -jd_UT - 1
            val eventDatePrev_LT = Math.floor(jd_UT + tz + eventTimePrev - 0.5) + 0.5
            if (jdToday_LT == eventDatePrev_LT) eventTime = eventTimePrev
            val eventTimeNext =
                celestialHoursToEarthTime * (normalizeRadians(ra + angh - lst) + TWO_PI)
            val eventDateNext_LT = Math.floor(jd_UT + tz + eventTimeNext - 0.5) + 0.5
     ...
     */
    private fun refraction(alt: Double): Double {
        var alt = alt
        ...
        var niter = 0
        do {
            val altOut = computeGeometricElevation(alt)
     ...
     * achieved in method [.refraction] by iteration.
     * @param alt Apparent elevation in radians.
     * @return Geometric elevation in radians.
     */
    private fun computeGeometricElevation(alt: Double): Double {
        val Ps = 1010.0 // Pressure in mb
        val Ts = 10 + 273.15 // Temperature in K
        val altDeg = alt * RAD_TO_DEG
        ...
        val r =
            DEG_TO_RAD * Math.abs(Math.tan(PI_OVER_TWO - (altDeg + 7.31 / (altDeg + 4.4)) * DEG_TO_RAD)) / 60.0
        val refr = r * (0.28 * Ps / Ts)
     ...
     */
    protected fun getIlluminationPhase(body: Ephemeris?) {
        val dlon = body!!.rightAscension - sun!!.rightAscension
        val cosElong = Math.sin(sun!!.declination) * Math.sin(
            body.declination
        ) +
                Math.cos(sun!!.declination) * Math.cos(body.declination) * Math.cos(dlon)
        val RE = sun!!.distance
        val RO = body.distance
        // Use elongation cosine as trick to solve the rectangle and get RP (distance body - sun)
        val RP = Math.sqrt(-(cosElong * 2.0 * RE * RO - RE * RE - RO * RO))
        val DPH = (RP * RP + RO * RO - RE * RE) / (2.0 * RP * RO)
    ...
    protected fun toJulianDay(
        ...
        if (year < 1582 || year == 1582 && month < 10 || year == 1582 && month == 10 && day < 15) julian =
            true
        var M = month
        var Y = year
        if (M < 3) {
        ...
        val A = Y / 100
        val B = if (julian) 0 else 2 - A + A / 4
        val dayFraction = (h + (m + s / 60.0) / 60.0) / 24.0
        return dayFraction + (365.25 * (Y + 4716)).toInt() + (30.6001 * (M + 1)).toInt() + day + B - 1524.5
    }
     ...
     */
    private fun obtainAccurateRiseSetTransit(
        riseSetJD: Double,
        index: EVENT,
        niter: Int,
        sun: Boolean
    ): Double {
        var riseSetJD = riseSetJD
        var step = -1.0
        for (i in 0 until niter) {
            if (riseSetJD == -1.0) return riseSetJD // -1 means no rise/set from that location
            setUTDate(riseSetJD)
        ...
        return if (step > 1.0 / SECONDS_PER_DAY) (-1).toDouble() else riseSetJD // did not converge => without rise/set/transit in this date
    }
    ...
    companion object {
        ...
        const val LIGHT_TIME_DAYS_PER_AU = 0.00577551833109
        ...
        private val sun_elements = arrayOf(
            doubleArrayOf(403406.0, 0.0, 4.721964, 1.621043),
            doubleArrayOf(195207.0, -97597.0, 5.937458, 62830.348067),
            doubleArrayOf(119433.0, -59715.0, 1.115589, 62830.821524),
            doubleArrayOf(112392.0, -56188.0, 5.781616, 62829.634302),
            doubleArrayOf(3891.0, -1556.0, 5.5474, 125660.5691),
            doubleArrayOf(2819.0, -1126.0, 1.512, 125660.9845),
            doubleArrayOf(1721.0, -861.0, 4.1897, 62832.4766),
            doubleArrayOf(0.0, 941.0, 1.163, .813),
            doubleArrayOf(660.0, -264.0, 5.415, 125659.31),
            doubleArrayOf(350.0, -163.0, 4.315, 57533.85),
            doubleArrayOf(334.0, 0.0, 4.553, -33.931),
            doubleArrayOf(314.0, 309.0, 5.198, 777137.715),
            doubleArrayOf(268.0, -158.0, 5.989, 78604.191),
            doubleArrayOf(242.0, 0.0, 2.911, 5.412),
            doubleArrayOf(234.0, -54.0, 1.423, 39302.098),
            doubleArrayOf(158.0, 0.0, .061, -34.861),
            doubleArrayOf(132.0, -93.0, 2.317, 115067.698),
            doubleArrayOf(129.0, -20.0, 3.193, 15774.337),
            doubleArrayOf(114.0, 0.0, 2.828, 5296.67),
            doubleArrayOf(99.0, -47.0, .52, 58849.27),
            doubleArrayOf(93.0, 0.0, 4.65, 5296.11),
            doubleArrayOf(86.0, 0.0, 4.35, -3980.7),
            doubleArrayOf(78.0, -33.0, 2.75, 52237.69),
            doubleArrayOf(72.0, -32.0, 4.5, 55076.47),
            doubleArrayOf(68.0, 0.0, 3.23, 261.08),
            doubleArrayOf(64.0, -10.0, 1.22, 15773.85),
            doubleArrayOf(46.0, -16.0, .14, 188491.03),
            doubleArrayOf(38.0, 0.0, 3.44, -7756.55),
            doubleArrayOf(37.0, 0.0, 4.37, 264.89),
            doubleArrayOf(32.0, -24.0, 1.14, 117906.27),
            doubleArrayOf(29.0, -13.0, 2.84, 55075.75),
            doubleArrayOf(28.0, 0.0, 5.96, -7961.39),
            doubleArrayOf(27.0, -9.0, 5.09, 188489.81),
            doubleArrayOf(27.0, 0.0, 1.72, 2132.19),
            doubleArrayOf(25.0, -17.0, 2.56, 109771.03),
            doubleArrayOf(24.0, -11.0, 1.92, 54868.56),
            doubleArrayOf(21.0, 0.0, .09, 25443.93),
            doubleArrayOf(21.0, 31.0, 5.98, -55731.43),
            doubleArrayOf(20.0, -10.0, 4.03, 60697.74),
            doubleArrayOf(18.0, 0.0, 4.27, 2132.79),
            doubleArrayOf(17.0, -12.0, .79, 109771.63),
            doubleArrayOf(14.0, 0.0, 4.24, -7752.82),
            doubleArrayOf(13.0, -5.0, 2.01, 188491.91),
            doubleArrayOf(13.0, 0.0, 2.65, 207.81),
            doubleArrayOf(13.0, 0.0, 4.98, 29424.63),
            doubleArrayOf(12.0, 0.0, .93, -7.99),
            doubleArrayOf(10.0, 0.0, 2.21, 46941.14),
            doubleArrayOf(10.0, 0.0, 3.59, -68.29),
            doubleArrayOf(10.0, 0.0, 1.5, 21463.25),
            doubleArrayOf(10.0, -9.0, 2.55, 157208.4)
        )
         ...
         */
        protected fun getDate(jd: Double): IntArray {
            // The conversion formulas are from Meeus,
            // Chapter 7.
            val Z = Math.floor(jd + 0.5)
            val F = jd + 0.5 - Z
            var A = Z
            if (Z >= 2299161.0) {
                val a = ((Z - 1867216.25) / 36524.25).toInt()
                A += (1 + a - a / 4).toDouble()
            }
            val B = A + 1524
            ...
            val m = (h - hour) * 60.0
            val minute = m.toInt()
            val second = ((m - minute) * 60.0)
            return intArrayOf(year, month, day, hour, minute, second.toInt())
        }
         ...
         * @return The String.
         */
        fun getDateAsString(jd: Double): String {
            if (jd == -1.0) return "NO RISE/SET/TRANSIT FOR THIS OBSERVER/DATE"
            val date = getDate(jd)
            var zyr = ""
            var zmo = ""
            var zh = ""
            ...
            if (date[4] < 10) zm = "0"
            if (date[5] < 10) zs = "0"
            return date[0].toString() + "/" + zyr + date[1] + "/" + zmo + date[2] + " " + zh + date[3] + ":" + zm + date[4] + ":" + zs + date[5] + " UT"
        }
         ...
         */
        protected fun normalizeRadians(r: Double): Double {
            var r = r
        ...
        fun main(args: Array<String>) {
            ...
            try {
                ...
                val degSymbol = "\u00b0"
                println("Sun")
                println(" Az:       " + (smc.sun?.azimuth?.times(RAD_TO_DEG)) as Float + degSymbol)
                println(" El:       " + (smc.sun!!.elevation * RAD_TO_DEG) as Float + degSymbol)
                println(" Dist:     " + smc.sun!!.distance as Float + " AU")
                println(" RA:       " + (smc.sun!!.rightAscension * RAD_TO_DEG) as Float + degSymbol)
                println(" DEC:      " + (smc.sun!!.declination * RAD_TO_DEG) as Float + degSymbol)
                println(" Ill:      " + smc.sun!!.illuminationPhase as Float + "%")
                println(" ang.R:    " + (smc.sun!!.angularRadius * RAD_TO_DEG) as Float + degSymbol)
